...
const Table = () =>{
    ...
    const toggleActionButton = useRef<HTMLDivElement>(null);
    const filterButton = useRef<HTMLButtonElement>(null);
    const toggleFilterButton = useRef<HTMLDivElement>(null);
    const [users,setUsers] = useState([]);
    const [filteredUsers,setFilteredUsers] = useState([]);
    // const threeDotButton = useRef<HTMLButtonElement>(null);
    // const toggleThreeDotButton = useRef<HTMLDivElement>(null);
    const [threeDotButton,setThreeDotButton] = useState(-1);
    ...
    useEffect(()=>{
        const handleButtonClick = () => {
            if(toggleActionButton.current && toggleFilterButton.current){
                toggleActionButton.current.classList.toggle('hidden');
            }
        };
        if(actionButton.current){
            actionButton.current.addEventListener('click', handleButtonClick);
        }
        return () => {
            
            (actionButton.current)?actionButton.current.removeEventListener('click', handleButtonClick):"";
        };
    },[actionButton])
    ...
    useEffect(() => {
        ...
        const handleButtonClick = () => {
            if (toggleFilterButton.current && toggleActionButton.current) {
                toggleFilterButton.current.classList.toggle('hidden');
            }
        };
        if (filterButton.current) {
            filterButton.current.addEventListener('click', handleButtonClick);
        }
        return () => {

            (filterButton.current) ? filterButton.current.removeEventListener('click', handleButtonClick) : "";
        };
    }, [filterButton])
                        ...
                        <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                            ...
                            <div className="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                                ...
                                <div className="flex items-center space-x-3 w-full md:w-auto relative">
                                    <button ref={actionButton}  id="actionsDropdownButton" data-dropdown-toggle="actionsDropdown" className="w-full md:w-auto flex items-center justify-center py-2 px-4 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700" type="button">
                                        <svg className="-ml-1 mr-1.5 w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                                            <path clip-rule="evenodd" fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" />
                                        </svg>
                                        Actions
                                    </button>
                                    <div ref={toggleActionButton} id="actionsDropdown" className="absolute hidden left-[0px] top-[50px] z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600 duration-200">
                                    ...
                                    <button ref={filterButton} id="filterDropdownButton" data-dropdown-toggle="filterDropdown" className="w-full md:w-auto flex items-center justify-center py-2 px-4 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700" type="button">
                                    ...
                                    <div ref={toggleFilterButton} id="filterDropdown" className="absolute top-[50px] left-[0px] z-10 hidden w-48 p-3 bg-white rounded-lg shadow dark:bg-gray-700">
                            ...
                            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                    <tr>
                                        ...
                                        <th scope="col" className="px-4 py-3">Departent</th>
                                                ...
                                                <>
                                                    <tr className="border-b dark:border-gray-700">
                                                        ...
                                                        <td className="px-4 py-3 flex items-center justify-end relative">
                                                            <button onClick={()=>{
                                                                threeDotButton === index ? setThreeDotButton(-1) : setThreeDotButton(index)}
                                                                }  id="apple-imac-27-dropdown-button" data-dropdown-toggle="apple-imac-27-dropdown" className="inline-flex items-center p-0.5 text-sm font-medium text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                                                                <svg className="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                                                    <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                                                                </svg>
                                                            </button>
                                                            <div id="apple-imac-27-dropdown" className={`${threeDotButton === index ? '' : 'hidden'} absolute right-[40px] top-[20px] z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600`}>
